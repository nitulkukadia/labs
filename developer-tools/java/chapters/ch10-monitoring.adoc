...
+
.. Container name
.. Percent CPU utilization (the percentage of the host’s CPU and memory the container is using)
.. Total memory usage vs amount available to the container
.. Percent memory utilization
.. Network activity (The amount of data the container has sent and received over its network interface)
.. Disk activity (The amount of data the container has read to and written from block devices on the host)
.. PIDS??
+
